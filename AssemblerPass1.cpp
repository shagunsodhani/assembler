...
// Fourth Column Operation Field - format of operation
// Fifth Format Specification  #0 @1 normally 2
// Six Final Opcode
...
string CheckFormat(string a)
{
    ...
    else if(CheckOPTAB(a))// Checks whether it is operation or not and then return where it is Format 3 OR Format 2
         {
             for(int i=0;i<45;i++)
             if(OPTAB[i][0]==a)
             return OPTAB[i][2];
...
int main()
{
    ...
    in.open("work.txt");
    getline(in,str);
    int f=0,h=1,count=0;//count is no
    while(str!="END")
    {
     ...
     for(int i=1;i<count;i++)
     {
        ...
        switch(label)
        {
            ...
            case 3:
            {
                ...
                if(CheckDir_Indir(n[i][3])==2&&!CheckSYMTAB(n[i][3]))  //
                   {
                       Insert_SYMTAB(n[i][3]);
                       n[i][5]="0";
                       if(n[i][1]!="")
    ...
    for(int i=1;i<count;i++)
    {
         if(n[i][5]=="1") //Immediate Addressing
         {
             ...
             n[i][6]=(convert(OPCode(n[i][2]),"1")+"0"+n[i][6]);
         }
         else if(n[i][5]=="2")  // Indirect Addressing
         {
             ...
             //temp1=difference(SYMField(n[i][6]),n[i+1][0]);
             //temp1=temp1.substr(1,temp1.length());
             n[i][6]=(convert(OPCode(n[i][2]),"2")+"0"+n[i][6]);
         }
         else if(n[i][4]=="4") //format 4
         {
            n[i][6]=convert(OPCode(n[i][2].substr(1,n[i][3].length())),"3")+"10"+SYMField(n[i][3]);
         }
         else if(n[i][4]=="3")//format 3
         {
             ...
             //temp1=temp1.substr(1,temp1.length());
             string temp1;
             string temp=convert(OPCode(n[i][2]),"3");
             temp=temp+"2";
